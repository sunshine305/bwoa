...
function save(eId) {
		...
		$.ajax({
		  	url : "updateRole.do?no="+no,   //要提交的URL路径
			type : "post",      //发送请求的方式
	...
	function searchByName(page){
		...
		//alert(name+";"+dqid);	
		window.location.href="rolePageContentByName.do?name="+name+"&areaId="+dqid+"&page="+page;
	}
	...
	function turnPage(areaId,pageCount){
		...
		$.ajax({
		  	url : "showAreaRole.do?nowPage="+pageCount+"&name="+name,   //要提交的URL路径
			type : "post",      //发送请求的方式
