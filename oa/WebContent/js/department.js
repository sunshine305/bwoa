 ...
 */
		...
		function update(dId, name, status) {
			window.location.href = "department/setStatus.do?departmentId=" + dId + "&name="
					+ name + "&status=" + status;
		}
		//查询
		function query() {
			var name = document.getElementById('queryName').value;
			window.location.href = "department/query.do?name=" + name;
		}
		//编辑
		...
		function updateName(dId) {
			...
			var status = 1;
			window.location.href = "department/update.do?departmentId=" + dId + "&name="
					+ name + "&status=" + status;
		}
		//撤销
		function reset(dId) {
		...
		function delet(dId) {
			//alert(dId);	
			if(dId==-1){
				window.location.href = "department/delet.do?departmentId=" + dIdDT;	
			}else{
				window.location.href = "department/delet.do?departmentId=" + dId;
			}
		}